...
Das erzeugt dort die komplette Zettelkasten-Struktur, also `Zettelkasten`, `daily todos`, `Vorlagen` und `Architektur Decision Record`, und schreibt den Pfad nach `~/.config/obsidian-vault`.
...
- **Obsidian:** Vault-Ort kommt aus `~/.config/obsidian-vault`. Bilder landen über `:ObsidianImgWeek` oder `<Space>oi` im Wochenordner `daily todos/JAHR/kwWOCHE`, der Pfad wird dafür zur Laufzeit gesetzt, weil das Plugin `img_folder` nur als String akzeptiert.
